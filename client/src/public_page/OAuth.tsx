...
const OAuth = () => {
  ...
  const handleGoogleClick = async () => {
    try {
      ...
      //this run after redirecting back to the app
      const googleResults = await signInWithRedirect(auth, provider);

      if (!googleResults) {
        throw new Error('No redirect result found.');
      }

      const { user } = googleResults;
  ...
  return (
    <button type="button" className="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800" onClick={handleGoogleClick}>
